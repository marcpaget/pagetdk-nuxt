...
function defineRenderHandler(handler) {
    return eventHandler(async (event) => {
        if (event.node.req.url.endsWith('/favicon.ico')) {
            ...
            event.node.res.end('data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7')
            return
        }
        ...
        if (!response) {
            if (!event.node.res.writableEnded) {
                ...
                event.node.res.end('No response returned from render handler: ' + event.node.req.url)
            }
            return
        }
            ...
            switch (type) {
                ...
                default:
                    ...
                    if (
                        ...
                        log(
                            `Cannot stringify POJOs with symbolic keys ${Object.getOwnPropertySymbols(thing).map(
        ...
        switch (type) {
            ...
            default:
                ...
                if (Object.getPrototypeOf(thing) === null) {
                    ...
                    return `Object.create(null,{${Object.keys(thing)
                        .map((key) => `${safeKey(key)}:{writable:true,enumerable:true,value:${stringify(thing[key])}}`)
                        .join(',')}})`
                }
...
const renderer = defineRenderHandler(async (event) => {
    ...
    const htmlContext = {
        ...
        bodyPrepend: normalizeChunks([renderedMeta.bodyScriptsPrepend, ssrContext.teleports?.body]),
        body: [_rendered.html],
...
<body ${joinAttrs(html.bodyAttrs)}>${joinTags(html.bodyPrepend)}${joinTags(html.body)}${joinTags(
        html.bodyAppend
    )}</body>
</html>`
}
