...
const sliceSimulatorCom = require('@prismicio/slice-simulator-com');
...
const LibrariesStateLike = t__namespace.record(
  t__namespace.string,
  t__namespace.intersection([
    ...
    t__namespace.type({
      components: t__namespace.record(
        t__namespace.string,
        t__namespace.intersection([
          t__namespace.type({
            id: t__namespace.string,
            model: t__namespace.intersection([
              t__namespace.type({
                id: t__namespace.string,
                variations: t__namespace.array(
                  t__namespace.intersection([
                    t__namespace.type({ id: t__namespace.string }),
                    t__namespace.partial({ name: t__namespace.string })
                  ])
                )
              }),
              t__namespace.partial({ name: t__namespace.string })
            ]),
            mocks: t__namespace.record(t__namespace.string, t__namespace.type({ variation: t__namespace.string }))
          }),
          t__namespace.partial({
            name: t__namespace.string
          })
...
class CoreManager {
  ...
  async _initAPI() {
    this._api = new sliceSimulatorCom.SimulatorAPI({
      [sliceSimulatorCom.ClientRequestType.GetLibraries]: (_req, res) => {
        return res.success(this.stateManager.getLibraries());
      },
      [sliceSimulatorCom.ClientRequestType.SetSliceZone]: (req, res) => {
        this.stateManager.setSliceZone(req.data);
        return res.success();
      },
      [sliceSimulatorCom.ClientRequestType.SetSliceZoneFromSliceIDs]: (req, res) => {
        this.stateManager.setSliceZoneFromSliceIDs(req.data);
        return res.success();
      },
      [sliceSimulatorCom.ClientRequestType.ScrollToSlice]: (req, res) => {
        var _a;
      ...
      async (activeSlice) => {
        if (this._api) {
          try {
            ...
            if (error instanceof sliceSimulatorCom.ResponseError && error.response.status === 400) {
...
exports.StateManagerEventType = StateManagerEventType;
exports.StateManagerStatus = StateManagerStatus;
exports.disableEventHandler = disableEventHandler;
exports.getActiveSliceDOM = getActiveSliceDOM;
exports.getDefaultManagedState = getDefaultManagedState;
exports.getDefaultMessage = getDefaultMessage;
exports.getDefaultProps = getDefaultProps;
exports.getDefaultSlices = getDefaultSlices;
...
exports.sliceSimulatorAccessedDirectly = sliceSimulatorAccessedDirectly;
