...
// node_modules/@unhead/schema-org-vue/dist/index.mjs
var name = "@unhead/schema-org-vue";
...
var provideResolver = (input, resolver2) => {
  if (!input)
    input = {};
  input._resolver = resolver2;
  return input;
};
var defineAddress = (input) => provideResolver(input, "address");
var defineAggregateOffer = (input) => provideResolver(input, "aggregateOffer");
var defineAggregateRating = (input) => provideResolver(input, "aggregateRating");
var defineArticle = (input) => provideResolver(input, "article");
var defineBreadcrumb = (input) => provideResolver(input, "breadcrumb");
var defineComment = (input) => provideResolver(input, "comment");
var defineEvent = (input) => provideResolver(input, "event");
var defineVirtualLocation = (input) => provideResolver(input, "virtualLocation");
var definePlace = (input) => provideResolver(input, "place");
var defineHowTo = (input) => provideResolver(input, "howTo");
var defineHowToStep = (input) => provideResolver(input, "howToStep");
var defineImage = (input) => provideResolver(input, "image");
var defineJobPosting = (input) => provideResolver(input, "jobPosting");
var defineLocalBusiness = (input) => provideResolver(input, "localBusiness");
var defineOffer = (input) => provideResolver(input, "offer");
var defineOpeningHours = (input) => provideResolver(input, "openingHours");
var defineOrganization = (input) => provideResolver(input, "organization");
var definePerson = (input) => provideResolver(input, "person");
var defineProduct = (input) => provideResolver(input, "product");
var defineQuestion = (input) => provideResolver(input, "question");
var defineRecipe = (input) => provideResolver(input, "recipe");
var defineReview = (input) => provideResolver(input, "review");
var defineVideo = (input) => provideResolver(input, "video");
var defineWebPage = (input) => provideResolver(input, "webPage");
var defineWebSite = (input) => provideResolver(input, "webSite");
var defineBook = (input) => provideResolver(input, "book");
var defineCourse = (input) => provideResolver(input, "course");
var defineItemList = (input) => provideResolver(input, "itemList");
var defineListItem = (input) => provideResolver(input, "listItem");
var defineMovie = (input) => provideResolver(input, "movie");
var defineSearchAction = (input) => provideResolver(input, "searchAction");
var defineReadAction = (input) => provideResolver(input, "readAction");
var defineSoftwareApp = (input) => provideResolver(input, "softwareApp");
var defineBookEdition = (input) => provideResolver(input, "bookEdition");
var isSPA = null;
...
var defineSchemaOrgComponent = (name2, defineFn) => {
...
var SchemaOrgDebug = defineComponent({
  name: "SchemaOrgDebug",
  props: {
    console: {
      type: Boolean,
    ...
    onMounted(() => {
      nextTick(() => {
        ...
        const fetchSchema = () => {
          ...
          schemaRaw.value = ($el == null ? void 0 : $el.textContent) || "";
        };
...
var idReference = (node) => ({
  "@id": typeof node !== "string" ? node["@id"] : node
});
var resolvableDateToDate = (val) => {
  try {
...
var setIfEmpty = (node, field, value) => {
  if (!(node == null ? void 0 : node[field]) && value)
    node[field] = value;
};
var asArray2 = (input) => Array.isArray(input) ? input : [input];
var dedupeMerge = (node, field, value) => {
  const dedupeMerge2 = [];
  const input = asArray2(node[field]);
...
var resolveDefaultType = (node, defaultType) => {
  const val = node["@type"];
  if (val === defaultType)
    return;
  const types = /* @__PURE__ */ new Set([
    ...asArray2(defaultType),
    ...asArray2(val)
  ]);
  node["@type"] = types.size === 1 ? val : [...types.values()];
};
var resolveWithBase = (base, urlOrPath) => {
  if (!urlOrPath || hasProtocol(urlOrPath) || !urlOrPath.startsWith("/") && !urlOrPath.startsWith("#"))
    return urlOrPath;
  return withBase(urlOrPath, base);
};
var resolveAsGraphKey = (key) => {
  if (!key)
    return key;
  return key.substring(key.lastIndexOf("#"));
};
var stripEmptyProperties = (obj) => {
...
var aggregateOfferResolver = defineSchemaOrgResolver({
  ...
  resolve(node, ctx) {
    ...
    if (node.offers)
      setIfEmpty(node, "offerCount", asArray2(node.offers).length);
    return node;
  }
});
...
var articleResolver = defineSchemaOrgResolver({
  ...
  resolveRootNode(node, { find, meta }) {
    ...
    if (node.image && !node.thumbnailUrl) {
      const firstImage = asArray2(node.image)[0];
...
var videoResolver = defineSchemaOrgResolver({
  ...
  resolveRootNode(video, { find }) {
    ...
    if (video.image && !video.thumbnailUrl) {
      const firstImage = asArray2(video.image)[0];
...
var questionResolver = defineSchemaOrgResolver({
  ...
  resolveRootNode(question, { find }) {
    const webPage = find(PrimaryWebPageId);
    if (webPage && asArray2(webPage["@type"]).includes("FAQPage"))
...
var resolveNode = (node, ctx, resolver2) => {
  var _a;
  ...
  (_a = resolver2.inheritMeta) == null ? void 0 : _a.forEach((entry) => {
...
var resolveNodeId = (node, ctx, resolver2, resolveAsRoot = false) => {
  var _a, _b;
  ...
  if (!node["@id"]) {
    ...
    if (!alias) {
      const type = ((_b = asArray2(node["@type"])) == null ? void 0 : _b[0]) || "";
    ...
    node["@id"] = prefixId(ctx.meta[prefix], `#/schema/${alias}/${hashCode2(JSON.stringify(hashNodeData))}`);
  }
  return node;
};
function resolveRelation(input, ctx, fallbackResolver, options = {}) {
  if (!input)
    return input;
  const ids = asArray2(input).map((a) => {
...
var groupBy = (array, predicate) => array.reduce((acc, value, index, array2) => {
...
//# sourceMappingURL=@unhead_schema-org-vue.js.map
