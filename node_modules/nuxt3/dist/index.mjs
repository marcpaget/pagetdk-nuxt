...
import { useNuxt, resolveFiles, defineNuxtModule, addPlugin, addTemplate, addComponent, updateTemplates, addVitePlugin, addWebpackPlugin, findPath, addImportsSources, tryResolveModule, isIgnored, resolveAlias, addPluginTemplate, logger, normalizeModuleTranspilePath, resolvePath, createResolver, nuxtCtx, installModule, loadNuxtConfig, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils } from '@nuxt/kit';
...
function createTransformPlugin(nuxt, getComponents, mode) {
  ...
  function getComponentsImports() {
    ...
    return components.flatMap((c) => {
      const withMode = (mode2) => mode2 ? `${c.filePath}${c.filePath.includes("?") ? "&" : "?"}nuxt_component=${mode2}` : c.filePath;
      return [
        {
          as: c.pascalName,
          from: withMode(c.mode === "client" ? "client" : void 0),
          name: "default"
        },
        {
  ...
  return createUnplugin(() => ({
    ...
    async transform(code, id) {
      ...
      return {
        code: result.code,
        map: nuxt.options.sourcemap ? result.s.generateMap({ hires: true }) : void 0
      };
    }
  }));
}
...
const version = "3.5.0-28053630.1f38863c";
...
async function initNuxt(nuxt) {
  ...
  for (const config2 of nuxt.options._layers.map((layer) => layer.config).reverse()) {
    const layerModules = await resolveFiles(config2.srcDir, [
      `${config2.dir?.modules || "modules"}/*{${nuxt.options.extensions.join(",")}}`,
      `${config2.dir?.modules || "modules"}/*/index{${nuxt.options.extensions.join(",")}}`
    ]);
    for (const mod of layerModules) {
      watchedPaths.add(relative(config2.srcDir, mod));
  ...
  if (nuxt.options._generate && nuxt.options.experimental.payloadExtraction === void 0) {
...
async function loadNuxt(opts) {
  ...
  if (options.devtools === true || options.devtools && options.devtools.enabled !== false) {
    if (await import('./chunks/features.mjs').then((r) => r.ensurePackageInstalled(options.rootDir, "@nuxt/devtools", options.modulesDir))) {
  ...
  if (nuxt.options.experimental.watcher === "parcel") {
    ...
    if (watcherPath) {
      ...
      for (const layer of nuxt.options._layers) {
        ...
        const watcher2 = subscribe(layer.config.srcDir, (err, events) => {
          if (err) {
            return;
          }
