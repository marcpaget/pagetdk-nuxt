<template>
  <div class="flex flex-col mt-2">
    ...
    <p class="text-2xl font-bold font-aptos">
      Søg efter Mocs du kan bygge med sætnr.:
    </p>
    <input
      type="text"
    ...
    <div class="grid grid-cols-3 gap-4">
      <div
        v-for="lego in res.results"
        :key="lego.id"
        class="bg-gray-100 p-4 rounded-lg"
      >
        <NuxtLink
          :to="lego.moc_url"
          class="text-xl font-bold font-aptos"
        >
          {{ lego.name }}
        </NuxtLink>
        <NuxtImg
          :src="lego.moc_img_url"
          :alt="lego.name"
          class="h-8 w-8"
        />
      </div>
